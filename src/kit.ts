...
export class PasskeyKit extends PasskeyBase {
  ...
  public async connectWallet(
    ...
    if (!this.keyId) this.keyId = keyId;
    ...
    let contractId: string | undefined = walletPublicKey
      ? this.encodeContract(walletPublicKey, keyIdBuffer)
      : this.encodeContract(this.walletPublicKey, keyIdBuffer);
  ...
  public async signAuthEntry(
    ...
    switch (credentials.signature().switch().name) {
      case 'scvVoid':
        credentials.signature(xdr.ScVal.scvVec([xdr.ScVal.scvMap([scEntry])]));
        break;
      case 'scvVec':
  ...
  public async sign<T>(
    ...
    if (!(txn instanceof AssembledTransaction)) {
      try {
        txn = AssembledTransaction.fromXDR(
          this.wallet!.options,
    ...
    return this.wallet![fn](
      {
        signer: {
          tag: 'Policy',
          values: [policy, [expiration], this.getSignerLimits(limits), { tag: store, values: undefined }],
        },
      },
      {
        timeoutInSeconds: this.timeoutInSeconds,
  ...
  private getSignerLimits(limits: SignerLimits) {
    if (!limits) return [undefined] as SDKSignerLimits;
  ...
  private encodeContract(walletPublicKey: string, keyIdBuffer: Buffer) {
    let contractId: string | undefined = StrKey.encodeContract(
      hash(
